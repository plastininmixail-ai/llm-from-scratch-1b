"""
Качает разнообразные instruct-датасеты (новые источники).

Источники:
- HuggingFaceH4/no_robots — курируемый instruct
- teknium/OpenHermes-2.5 — топ instruct dataset
- garage-bAInd/Open-Platypus — chain-of-thought
"""
...
def main() -> int:
    ...
    p.add_argument("--out-dir", type=Path, default=Path("data/instruct_v2"))
    p.add_argument("--target-mb", type=int, default=80)
    args = p.parse_args()
    args.out_dir.mkdir(parents=True, exist_ok=True)

    sources = [
        # No Robots — курируемый высококачественный instruct (10K)
        ("HuggingFaceH4/no_robots", None, "train_sft"),
        # Open-Platypus — chain-of-thought reasoning (25K)
        ("garage-bAInd/Open-Platypus", None, "train"),
        # OpenHermes-2.5 — топ instruct (1M, берём 50K)
